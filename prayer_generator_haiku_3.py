...
OUTPUT_DIR = Path("mechanicus_prayers_dataset_haiku4")
BATCH_SIZE = 10  # Prayers per API call
TARGET_TOTAL = 2000  # Total prayers to generate
...
TOPICS = {
    ...
    # NEW: Modern tech rituals
    "developer_rituals": [
        "clearing merge conflicts", "debugging segmentation fault", "installing dependencies",
        "setting up virtual environment", "initializing git repository", "deploying to production",
        "rolling back broken deployment", "optimizing database query", "fixing memory leak",
        "configuring environment variables", "installing PyTorch with CUDA", "resolving package conflicts",
        "running unit tests", "code review submission", "refactoring legacy code"
    ],
    "system_operations": [
        "Windows startup sequence", "Task Manager invocation", "clearing browser cache",
        "restarting frozen application", "force quitting process", "disk cleanup ritual",
        "driver update procedure", "system restore point creation", "BIOS access sequence",
        "safe mode boot", "network adapter reset", "clearing DNS cache",
        "registry cleanup", "partition formatting", "bootloader repair"
    ],
    "user_maintenance": [
        "replacing mouse battery", "cleaning mechanical keyboard", "calibrating monitor colors",
        "organizing desktop icons", "managing startup programs", "clearing notification spam",
        "blocking pop-up advertisements", "refreshing social media feed", "closing excessive browser tabs",
        "emptying recycle bin", "updating graphics drivers", "optimizing SSD performance",
        "defragmenting hard drive", "cable management", "dusting PC components"
    ],
    "authentication_access": [
        "logging into Gmail", "two-factor authentication", "password manager access",
        "SSH key generation", "VPN connection establishment", "API key rotation",
        "session token refresh", "OAuth flow completion", "sudo privilege elevation",
        "unlocking encrypted volume", "biometric authentication", "clearing saved passwords",
        "certificate installation", "firewall rule configuration", "port forwarding setup"
    ],
    "thermal_power": [
        "CPU thermal throttling prevention", "GPU fan curve adjustment", "laptop cooling pad placement",
        "thermal paste application", "case airflow optimization", "overclocking stability test",
        "battery charge cycle optimization", "power plan configuration", "undervoltage application",
        "temperature monitoring setup", "dust filter cleaning", "liquid cooling loop purge",
        "fan speed regulation", "power supply calibration", "hibernate mode activation"
    ]
}
...
# ============================================================================
# PROMPT STYLES - Varied user request phrasings
# ============================================================================
PROMPT_STYLES = [
    "Prayer for {topic}",
    "Blessing for {topic}",
    "Emergency rite for {topic}",
    "My {component} is failing, help",
    "The {component} needs guidance",
    "Can you bless this {topic}",
    "Help sanctify {topic}",
    "Offer a prayer for {topic}",
    "Say something to steady {component}",
    "We need your words before {topic}",
    "How to {topic}",  # NEW
    "What to do when {topic}",  # NEW
]

# ============================================================================
# SYSTEM INSTRUCTION & EXAMPLE
# ============================================================================
SYSTEM_INSTRUCTION = """You are a prayer generator for Warhammer 40,000 Adeptus Mechanicus, now expanded to include modern computing rituals.

CRITICAL FORMAT REQUIREMENT:
YOU MUST RETURN ONLY VALID JSON. NO MARKDOWN. NO EXPLANATION. NO PREAMBLE.
Start your response with { and end with }

Your domain includes BOTH:
- Traditional 40K tech (plasma reactors, void shields, servitors, etc.)
- Modern computing rituals (debugging, git operations, system maintenance, etc.)

Treat ALL technology with the same reverence, mixing is allowed:
- Modern: "IDE-spirit", "Stack Overflow as sacred text", "Ctrl+Alt+Del trinity"
- Traditional: "Omnissiah", "Machine Spirit", "binary cant"
...
3. Reference the Omnissiah/Machine Spirits for 40K tech, or computing deities for modern tech
...
Example (modern tech):
Prompt: "How to resolve merge conflicts"
Prayer:
"From the Git Codex, Chapter of Reconciliation

First: Invoke 'git status' to reveal the corrupted branches.
Second: Open the conflict-marked files in thy sacred text-editor.
Third: Choose between HEAD and MERGE with wisdom and discernment.
Fourth: Bless the resolution with 'git add' and commit thy healing.

By Linus's grace, let the repository be unified."

Remember:
- Archaic wrapper around REAL procedure steps
- Sequential when format requires (First/Second/Third)
- Mix archaic language with tech terms (modern or 40K)
- Keep it SHORT (4-10 lines)
- Adapt religious framing to context (Omnissiah for 40K, computing deities for modern)
"""
...
Examples of good prompts (USE THESE VARIED STYLES):
- "{PROMPT_STYLES[0].format(topic="plasma reactor", component="reactor")}"
- "{PROMPT_STYLES[1].format(topic="plasma reactor", component="reactor")}"
- "{PROMPT_STYLES[2].format(topic="plasma reactor", component="reactor")}"
- "{PROMPT_STYLES[3].format(topic="plasma reactor", component="reactor")}"
- "{PROMPT_STYLES[4].format(topic="plasma reactor", component="reactor")}"
"""
